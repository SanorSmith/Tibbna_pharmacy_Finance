...
export async function GET(
  ...
  try {
    ...
    const allLabOrders = await Promise.all(
      compositions.map(async (comp) => {
        try {
          ...
          console.log(`Checking composition ${comp.composition_uid}:`, JSON.stringify(details, null, 2));

          // Check if this composition contains lab order data - try multiple possible paths
          const serviceName = 
            details["template_clinical_encounter_v1/service_request/request/service_name|other"] as string ||
            details["template_clinical_encounter_v1/service_request/service_name|other"] as string ||
            details["service_name|other"] as string;
          
          const serviceTypeCode = 
            details["template_clinical_encounter_v1/service_request/request/service_type|code"] as string ||
            details["template_clinical_encounter_v1/service_request/service_type|code"] as string ||
            details["service_type|code"] as string;
          
          const serviceTypeValue = 
            details["template_clinical_encounter_v1/service_request/request/service_type|value"] as string ||
            details["template_clinical_encounter_v1/service_request/service_type|value"] as string ||
            details["service_type|value"] as string;
          
          const description = 
            details["template_clinical_encounter_v1/service_request/request/description"] as string ||
            details["template_clinical_encounter_v1/service_request/description"] as string ||
            details["description"] as string;
          
          const clinicalIndication = 
            details["template_clinical_encounter_v1/service_request/request/clinical_indication"] as string ||
            details["template_clinical_encounter_v1/service_request/clinical_indication"] as string ||
            details["clinical_indication"] as string;
          
          const urgency = 
            details["template_clinical_encounter_v1/service_request/request/urgency|value"] as string ||
            details["template_clinical_encounter_v1/service_request/urgency|value"] as string ||
            details["urgency|value"] as string;
          
          const requestedDate = 
            details["template_clinical_encounter_v1/service_request/request/requested_date"] as string ||
            details["template_clinical_encounter_v1/service_request/requested_date"] as string ||
            details["requested_date"] as string;
          
          const requestingProvider = 
            details["template_clinical_encounter_v1/service_request/request/requesting_provider"] as string ||
            details["template_clinical_encounter_v1/service_request/requesting_provider"] as string ||
            details["requesting_provider"] as string;
          
          const receivingProvider = 
            details["template_clinical_encounter_v1/service_request/request/receiving_provider"] as string ||
            details["template_clinical_encounter_v1/service_request/receiving_provider"] as string ||
            details["receiving_provider"] as string;
          
          const requestStatus = 
            details["template_clinical_encounter_v1/service_request/request/request_status|value"] as string ||
            details["template_clinical_encounter_v1/service_request/request_status|value"] as string ||
            details["request_status|value"] as string;
          
          const timing = 
            details["template_clinical_encounter_v1/service_request/request/timing"] as string ||
            details["template_clinical_encounter_v1/service_request/timing"] as string ||
            details["timing"] as string;
          
          const requestId = 
            details["template_clinical_encounter_v1/service_request/request_id"] as string ||
            details["template_clinical_encounter_v1/service_request_id"] as string ||
            details["request_id"] as string;
          
          const narrative = 
            details["template_clinical_encounter_v1/service_request/narrative"] as string ||
            details["template_clinical_encounter_v1/narrative"] as string ||
            details["narrative"] as string;

          console.log(`Extracted data - serviceName: ${serviceName}, clinicalIndication: ${clinicalIndication}`);

          // Only return if there's actual lab order data
          if (serviceName && clinicalIndication) {
            console.log(`Composition ${comp.composition_uid} has lab order: ${serviceName}`);
            return {
            ...
            console.log(`Skipping composition ${comp.composition_uid} - no lab order data`);
            return null;
          }
        } catch (error) {
    ...
    const validLabOrders = allLabOrders
      .filter((labOrder): labOrder is NonNullable<typeof labOrder> => labOrder !== null)
...
export async function POST(
  ...
  try {
    ...
    compositionData["template_clinical_encounter_v1/service_request/request/service_type|terminology"] = "SNOMED-CT";
    compositionData["template_clinical_encounter_v1/service_request/request/service_type|code"] = service_type_code || "104177005";
    compositionData["template_clinical_encounter_v1/service_request/request/service_type|value"] = service_type_value || "Complete blood count (procedure)";
    ...
    compositionData["template_clinical_encounter_v1/service_request/request/urgency|value"] = urgency.charAt(0).toUpperCase() + urgency.slice(1); // Capitalize first letter
    compositionData["template_clinical_encounter_v1/service_request/request/urgency|terminology"] = "local";
    compositionData["template_clinical_encounter_v1/service_request/request/urgency|code"] = urgency;
    ...
    compositionData["template_clinical_encounter_v1/service_request/request/request_status|terminology"] = "local";
    compositionData["template_clinical_encounter_v1/service_request/request/request_status|code"] = "ordered";
    compositionData["template_clinical_encounter_v1/service_request/request/request_status|value"] = "Ordered";
    compositionData["template_clinical_encounter_v1/service_request/request/timing"] = timing || eventTime;
    compositionData["template_clinical_encounter_v1/service_request/request_id"] = `labreq-${Date.now()}`;
    compositionData["template_clinical_encounter_v1/service_request/narrative"] = narrative || `${service_name} ordered due to ${clinical_indication}`;
