/**
 * Pharmacy Order Detail — Server Page
 * Only accessible to users with the "pharmacist" role.
 */
import { getUser } from "@/lib/user";
import { getUserWorkspaces } from "@/lib/db/queries/workspace";
import { redirect } from "next/navigation";
import PharmacyOrderDetailPage from "./order-detail";

interface PageProps {
  params: Promise<{ workspaceid: string; orderid: string }>;
}

export default async function PharmacyOrderDetailServerPage({ params }: PageProps) {
  ...
  return <PharmacyOrderDetailPage workspaceid={workspaceid} orderid={orderid} />;
}
