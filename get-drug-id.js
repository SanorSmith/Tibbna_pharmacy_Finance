...
async function getDrugId() {
  ...
  try {
    ...
    if (drugResult.rows.length > 0) {
      ...
      // Step 2: Show the connection to pharmacy order items
      ...
      // Step 3: Show the connection to inventory system via drug name
      ...
      if (itemsResult.rows.length > 0) {
        ...
        itemsResult.rows.forEach((row, index) => {
          console.log(`  ${index + 1}. item_id: ${row.id}`);
          console.log(`     name: ${row.name}`);
          
          // Step 4: Show the price connection
