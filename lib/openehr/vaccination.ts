...
export interface VaccinationComposition {
  ...
  // Use problem_diagnosis fields to store vaccination data with VACCINATION prefix
...
export async function createVaccination(
  ehrId: string,
  composition: VaccinationComposition
): Promise<string> {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      console.error("openEHR Vaccination Creation Error:");
...
export async function listVaccinations(ehrId: string): Promise<VaccinationListItem[]> {
  const url = `${process.env.EHRBASE_URL}/ehrbase/rest/openehr/v1/query/aql`;
  // Query for clinical encounters that have VACCINATION prefix in problem_diagnosis_name
...
export async function getVaccination(
...
): Promise<VaccinationComposition> {
  ...
  return response.data as VaccinationComposition;
}
